...
public class CountdownSessionTests
{
    ...
    [Fact]
    public void Refresh_AtEndTime_TransitionsToFinished()
    {
        var clock = new FakeClock();
        var sut = NewSession(clock);
    ...
    [Fact]
    public void Reset_FromAnyState_GoesIdleAndClearsTimingFields()
    {
        var clock = new FakeClock();
        var sut = NewSession(clock);
        sut.Start(TimeSpan.FromMinutes(1));
    ...
    [Fact]
    public void Start_FromFinished_RestartsImmediately()
    {
        var clock = new FakeClock();
        var sut = NewSession(clock);
        sut.Start(TimeSpan.FromMinutes(1));
